...
class SearchPhotoAdapter () :
    ...
    private var photos: ArrayList<Detail> = ArrayList()
    lateinit var photoClick: ((Detail) -> Unit)

    inner class HomePhotoVH(private val view: View) : RecyclerView.ViewHolder(view) {
        fun bind(photo: Detail) {
    ...
    fun submitData(list: List<Detail>){
        photos.addAll(list)
    ...
    private fun getItem(position: Int): Detail = photos[position]
